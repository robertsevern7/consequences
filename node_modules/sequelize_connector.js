...
var sequelize = new Sequelize('consequences', 'postgres', 'postgres',{
    dialect: 'postgres',
    port: 5432,
    host: '127.0.0.1'
});
...
var Section = sequelize.define('Section', {
    ...
    parent_section: {type: Sequelize.INTEGER, allowNull: true}
});

var NeverEndingStorySection = sequelize.define('NeverEndingStorySection', {
    id: {type: Sequelize.INTEGER, autoIncrement: true, primaryKey: true},
    content: {type: Sequelize.TEXT, allowNull: false},
    parent_section: {type: Sequelize.INTEGER, allowNull: true}
});
...
NeverEndingStorySection.belongsTo(User);
...
module.exports.getFullStory = function(storyId, page, includeEmailAddress, successHandler, failureHandler) {
    if (storyId == -1) {
        getNeverendingStory(page, successHandler, failureHandler);
    } else {
            ...
            if (story) {            
                story.getUser().success(function(user) {
                    ...
                    Section.findAll({
                        ...
                        createSectionsOutput(story, sections, includeEmailAddress, successHandler);                                        
                    }).error(function(err) {
...
var getNeverendingStory = function(page, successHandler, failureHandler) {
    var pageSize = 10;

    NeverEndingStorySection.count().success(function(totalStories) {
        var totalPages = Math.floor(totalStories/pageSize);
        page = page === 'end' ? totalPages : page;
        NeverEndingStorySection.findAll({
            order: 'id ASC',
            attributes: ['id', 'content', 'UserId'],
            offset: Math.max((page - 1), 0) * pageSize,
            limit: pageSize
        }).success(function(sections) {
            createSectionsOutput({
                title: 'The Neverending TalePipe', 
                id: -1,
                totalPages: totalPages,
                page: page
            }, sections, false, successHandler);                                        
        }).error(function(err) {
            console.log('Failed to get sections of neverending story')
            console.dir(e);
            failureHandler && failureHandler();
        });
    });
...
var createSectionsOutput = function(story, sections, includeEmailAddress, successHandler, failureHandler) {
    var outputSections = [];
    var userIds = [];
    ...
    if (!userIds.length) {
        story.sections = [];
        story.title = unescape(story.title);
        successHandler && successHandler(story);
        return;
    }
    ...
    User.findAll({
        ...
        for (var k = 0, len = sections.length; k < len; ++k) {
            ...
            outputSections.push({
                id: section.id,
                content: unescape(section.content),
...
function getUserStories(userId, pageNumber, pageSize, sortOrder, sortDir, successHandler, failureHandler) {
    ...
    sql = "SELECT t.* FROM \"Tales\" t INNER JOIN \"Sections\" s ON t.id = s.\"TaleId\" WHERE s.\"UserId\"=" + userId + ' ORDER BY ' + getTaleSortOrder(sortOrder) + ' ' + sortDir + ' OFFSET ' + offset + ' LIMIT ' + pageSize;
    ...
    Section.findAll({
        ...
        for (var j = 0, len = sections.length; j < len; ++j) {
            ...
            if (section.TaleId !== storyId) {
                ...
                storiesMap[storyId].sections[0].content = unescape(section.content);
            }
        }
...
module.exports.getStories = function(userId, pageNumber, pageSize, sortOrder, sortDir, successHandler, failureHandler) {
    ...
    console.log('Getting stories with:')
...
module.exports.createStory = function(story, user, content, successHandler, failureHandler) { 
    ...
    Tale.build(story).save().success(function(savedStory) {
        savedStory.setUser(user).success(function() {
            module.exports.createSection(content, undefined, savedStory, user, successHandler, failureHandler);
...
module.exports.createSection = function(sectionContent, lastSectionId, story, user, successHandler, failureHandler) {
    console.log('Saving story ' +  story.id);
    var isNeverending = story.id === -1;
    var databaseTable = isNeverending ? NeverEndingStorySection : Section; 

    var section = {
        content: escape(sectionContent)
    }

    if (lastSectionId && lastSectionId !== 'undefined') {
        section.parent_section = lastSectionId;
    }

    databaseTable.build(section).save().success(function(section) {
...
module.exports.lockStory = function(storyId, userId, successHandler, failureHandler) {
    console.log('locking a story')
    if (storyId%1 === 'NaN' || userId%1 === 'NaN') {
        failureHandler && failureHandler();
    }
    var sql = 'UPDATE "Tales" SET user_id_lock = ' + userId + ', locktime=NOW() WHERE (user_id_lock = ' + userId + ' OR user_id_lock IS NULL OR locktime < NOW() - INTERVAL \'20 MINUTES\') AND id=' + storyId;
...
module.exports.unlockStory = function(storyId, userId, successHandler, failureHandler) {
    ...
    sequelize.query(sql).on('success', function() {
...
module.exports.getRandomNames = function(successHandler, failureHandler) {
    var sql = "SELECT n.* FROM \"Names\" n ORDER BY RANDOM() LIMIT 2";
...
module.exports.getRandomLocation = function(successHandler, failureHandler) {
    sql = "SELECT loc.* FROM \"Locations\" loc ORDER BY RANDOM() LIMIT 1";
